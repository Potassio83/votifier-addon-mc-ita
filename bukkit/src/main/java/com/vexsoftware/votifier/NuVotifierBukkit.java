...
public class NuVotifierBukkit extends JavaPlugin implements VoteHandler, VotifierPlugin, ForwardedVoteListener {
    ...
    private boolean loadAndBind() {
        ...
        if(cfg.getBoolean("mc-italia.enable")){
            if(cfg.isSet("mc-italia.apiUrl")) {
                url = cfg.getString("mc-italia.apiUrl");
                new BukkitRunnable() {
                    @Override
                    public void run() {
                        ...
                        if (date.equalsIgnoreCase("00.10")) {
                            ...
                            try {
                                getUsers(url).forEach(s -> fireVotifierEvent(voteBuilder(s)));
                ...
                getLogger().log(Level.SEVERE, "Non hai impostato la stringa apiUrl");
            }
        }
        return true;
    }
